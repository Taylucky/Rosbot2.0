...
def main():
        ...
        for i, hashmap in enumerate(trainloader, 0):
            ...
            if i % logfreq == logfreq-1:  # print every 2000 mini-batches
                ...
                if (running_loss / logfreq) < lowest_loss:
                    print(f"New best model1! MSE loss: {running_loss / logfreq}")
                    model_name = f"./model1-fixnoise-{iteration}-best.pt"
                    print(f"Saving model to {model_name}")
                    torch.save(model, model_name)
                    lowest_loss = running_loss / logfreq
                running_loss = 0.0
        print(f"Finished {epoch=}")
        model_name = f"/u/syw3ev/project/Rosbot1207/training/DAVE2-Keras/model1-fixnoise-{iteration}-epoch{epoch}.pt"
        print(f"Saving model to {model_name}")
        torch.save(model, model_name)
        # if loss < 0.002:
    ...
    model_name = f'/u/syw3ev/project/Rosbot1207/training/DAVE2-Keras/model1-{iteration}.pt'
    torch.save(model, model_name)

    # delete models from previous epochs
    print("Deleting models from previous epochs...")
    for epoch in range(args.epochs):
        os.remove(f"/u/syw3ev/project/Rosbot1207/training/DAVE2-Keras/model1-fixnoise-{iteration}-epoch{epoch}.pt")
    print(f"Saving model to {model_name}")
    print("All done :)")
    time_to_train=time.time() - start_time
    print("Time to train: {}".format(time_to_train))
    # save metainformation about training
    with open(f'/u/syw3ev/project/Rosbot1207/training/DAVE2-Keras/model1-{iteration}-metainfo.txt', "w") as f:
